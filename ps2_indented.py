...
class Robot(object):
  ...
  """
  def __init__(self, room, speed=0, direction=0):
  ...
  def setRobotDirection(self, dirr):
    """
  ...
  def setRobotPosition(self, pos):
    ...
    """
    self.position.x = pos.getX()
    self.position.y = pos.getY()
...
class StandardRobot(Robot):
  ...
  def updatePositionAndClean(self):
    ...
    new = self.position.getNewPosition(self.getRobotDirection(), self.speed)
    while new:
      if self.room.isPositionInRoom(new):
        ...
        new = self.position.getNewPosition(newangle, self.speed)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
  ...
  for i in range(num_trials):
    ...
    while True:
      ...
      time_steps += 1
      if robot.room.getNumCleanedTiles()/float(robot.room.getNumTiles()) >= min_coverage:
        result.append(time_steps)
        break
  anim.done()
  return sum(result)/float(num_trials)
